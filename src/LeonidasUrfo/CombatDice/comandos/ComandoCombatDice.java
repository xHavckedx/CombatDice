		...
		if (args.length > 0) {
			if (args[0].equalsIgnoreCase("version")) {
				...
				if (jugadorstat.stream().map((n) -> n.getNombre()).collect(Collectors.toList()).contains(args[1])
						&& jugadorstat.stream().map((n) -> n.getNombre()).collect(Collectors.toList())
								.contains(jugador.getName())) {

					// si los jugadores existen, instancia una batalla, siempre y cuando el otro
					// jugador acepte
					...
					if (jugadorstat.get(intJugador1).isenCombate() == false
							&& jugadorstat.get(intJugador2).isenCombate() == false) {
					...
					if (Bukkit.getOnlinePlayers().stream().map(n -> n.getDisplayName()).collect(Collectors.toList())
							.contains(args[1])) {
				...
				if (jugadorstat.get(intJugador2).isenCombate() == false) {
					...
					Bukkit.broadcastMessage(ChatColor.BLUE
							...
							+ ChatColor.YELLOW + jugadorstat.get(intJugador1).getNombre() + ChatColor.BLUE + " y "
							+ ChatColor.YELLOW + jugadorstat.get(intJugador2).getNombre() + ChatColor.BLUE + " a "
							+ args[3] + " golpes");
					Bukkit.broadcastMessage("\n" + ChatColor.YELLOW + jugadorstat.get(intJugador1).getNombre()
							+ ChatColor.BLUE + " 0 - 0 " + ChatColor.YELLOW + jugadorstat.get(intJugador2).getNombre());
					...
					Bukkit.broadcastMessage(ChatColor.BOLD + jugadorstat.get(intJugador1).getNombre() + " ha tirado un "
							+ ChatColor.RED + tiradaJugador1);
					Bukkit.broadcastMessage(ChatColor.BOLD + jugadorstat.get(intJugador2).getNombre() + " ha tirado un "
							+ ChatColor.RED + tiradaJugador2);
					...
					if (tiradaJugador1 > tiradaJugador2) {
						...
						Bukkit.broadcastMessage(ChatColor.BOLD + jugadorstat.get(intJugador1).getNombre()
								+ " ha tirado un " + ChatColor.RED + tiradaJugador1);
						Bukkit.broadcastMessage(ChatColor.BOLD + jugadorstat.get(intJugador2).getNombre()
								+ " ha tirado un " + ChatColor.RED + tiradaJugador2);
						if (tiradaJugador1 > tiradaJugador2) {
							Bukkit.broadcastMessage(ChatColor.BOLD + "\nEmpieza el jugador "
									+ jugadorstat.get(intJugador1).getNombre());
							jugadorstat.get(intJugador1).setMiTurno(true);
						} else if (tiradaJugador2 > tiradaJugador1) {
							Bukkit.broadcastMessage(ChatColor.BOLD + "\nEmpieza el jugador "
									+ jugadorstat.get(intJugador2).getNombre());
							jugadorstat.get(intJugador2).setMiTurno(true);
						}
					}
				}
