...
public class ComandoCombatDiceStart {
	...
	public void onCommand(CommandSender sender, Command comando, String label, String[] args) {
		...
		if(args[2].matches("[+-]?\\d*(\\.\\d+)?")){
			...
			jugador.sendMessage(ChatColor.BLUE + "Enviando petición de combate a: " + jugador2.getName() + " con un número de " + golpes + " golpes ...");
			
			//Mensaje al contrincante para empezar el combate
			TextComponent Mensaje = new TextComponent( "Quieres aceptar el combate de " + jugador.getName() + " a " + golpes + " golpes? " );
			...
			//eventos el comando cd yes devuelve el jugador2 el jugador1 y el número de turnos
			//evento el comando cd no devuelve el jugador 1 y le quita el estado de pelea
			Aceptar.setClickEvent( new ClickEvent( ClickEvent.Action.RUN_COMMAND, "/cd yes " + jugador2.getName() + " " + jugador.getName() + " " + Integer.toString(golpes) ) );
			Rechazar.setClickEvent( new ClickEvent( ClickEvent.Action.RUN_COMMAND, "/cd no " + jugador2.getName() + " " + jugador.getName() ) );
